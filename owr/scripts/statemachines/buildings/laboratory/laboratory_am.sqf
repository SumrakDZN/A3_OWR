...
_labka animateSource ["hide_adv_opto", 1, true];
...
_labka setVariable ["ow_curr_res_cat", "", true];	// basic, weap, opto, siberite, comp
		...
		case 0: {
			...
			*/
			...
			if (_wip_progress >= 1.0) then {
				...
				[(_workers select 0), "Building finished", "Basic laboratory", mapGridPosition (getPos _labka)] remoteExec ["owr_fn_message", bis_curator_west];
			};
			...
			if (isNull (_labka getVariable "ow_build_wrhs")) then {
					...
					if (count crew _labka > 0) then {
						...
						if (((_labka getVariable "ow_curr_res_cat") == "basic") || (((_labka getVariable "ow_build_wrhs") getVariable "ow_wrhs_power_avl") >= ((_labka getVariable "ow_build_wrhs") getVariable "ow_wrhs_power_req"))) then {
							...
							switch ((_labka getVariable "ow_curr_res_cat")) do {
								case "basic": {
									_research_prog = (bis_curator_west getVariable "ow_am_res_basic_prog") select (_research_index);
									_research_compl = (bis_curator_west getVariable "ow_am_res_basic_comp") select (_research_index);
								};
								case "weap": {
									_research_prog = (bis_curator_west getVariable "ow_am_res_weap_prog") select (_research_index);
									_research_compl = (bis_curator_west getVariable "ow_am_res_weap_comp") select (_research_index);
								};
								case "opto": {
									_research_prog = (bis_curator_west getVariable "ow_am_res_opto_prog") select (_research_index);
									_research_compl = (bis_curator_west getVariable "ow_am_res_opto_comp") select (_research_index);
								};
								case "siberite": {
									_research_prog = (bis_curator_west getVariable "ow_am_res_siberite_prog") select (_research_index);
									_research_compl = (bis_curator_west getVariable "ow_am_res_siberite_comp") select (_research_index);
								};
								case "comp": {
									_research_prog = (bis_curator_west getVariable "ow_am_res_comp_prog") select (_research_index);
									_research_compl = (bis_curator_west getVariable "ow_am_res_comp_comp") select (_research_index);
								};
								default {};
							};
								...
								case "basic": {
									...
									for "_i" from 0 to (count _tempArray) do {
										if (_i != (_research_index)) then {
											_tempArray set [(_i), (bis_curator_west getVariable "ow_am_res_basic_prog") select _i];
										};
									};
									bis_curator_west setVariable ["ow_am_res_basic_prog", _tempArray, true];
								};
								case "weap": {
									...
									for "_i" from 0 to (count _tempArray) do {
										if (_i != (_research_index)) then {
											_tempArray set [(_i), (bis_curator_west getVariable "ow_am_res_weap_prog") select _i];
										};
									};
									bis_curator_west setVariable ["ow_am_res_weap_prog", _tempArray, true];
								};
								case "opto": {
									...
									for "_i" from 0 to (count _tempArray) do {
										if (_i != (_research_index)) then {
											_tempArray set [(_i), (bis_curator_west getVariable "ow_am_res_opto_prog") select _i];
										};
									};
									bis_curator_west setVariable ["ow_am_res_opto_prog", _tempArray, true];
								};
								case "siberite": {
									...
									for "_i" from 0 to (count _tempArray) do {
										if (_i != (_research_index)) then {
											_tempArray set [(_i), (bis_curator_west getVariable "ow_am_res_siberite_prog") select _i];
										};
									};
									bis_curator_west setVariable ["ow_am_res_siberite_prog", _tempArray, true];
								};
								case "comp": {
									...
									for "_i" from 0 to (count _tempArray) do {
										if (_i != (_research_index)) then {
											_tempArray set [(_i), (bis_curator_west getVariable "ow_am_res_comp_prog") select _i];
										};
									};
									bis_curator_west setVariable ["ow_am_res_comp_prog", _tempArray, true];
								};
								default {};
							};
							if (_research_prog >= 1.0) then {
								//[(crew _labka) select 0, "Research completed", [(_labka getVariable "ow_curr_res_cat"), _research_index, bis_curator_west] call owr_fn_getResearchName, mapGridPosition (getPos _labka)] spawn owr_fn_message;
								[(crew _labka) select 0, "Research completed", [(_labka getVariable "ow_curr_res_cat"), _research_index, bis_curator_west] call owr_fn_getResearchName, mapGridPosition (getPos _labka)] remoteExec ["owr_fn_message", bis_curator_west];
								_labka setVariable ["ow_curr_res_cat", "", true];
								_labka setVariable ["ow_lab_buildmode", 0, true];
								//hintSilent "done";
							} else {
								//hintSilent format ["lab is researching %1\nresearch #%2 prog %3 comp %4\n\nplayer scientist %5", (_labka getVariable "ow_curr_res_cat"), (_labka getVariable "ow_curr_res_index"), _research_prog, _research_compl, bis_curator_west getVariable "ow_skill_scientist"];
							};
						};
					};
				};
				...
				if (((_labka getVariable "ow_lab_right") != "") && (!_upgrade_right)) then {
					...
					switch (_labka getVariable "ow_lab_right") do {
						...
						case "opto": {
							_labka animateSource ["hide_adv_opto", 0, true];
							_powerReq = _powerReq + 25;
						};
						default {};
					};
		...
		case 3: {
			...
			*/
			...
			if (_wip_progress >= 1.0) then {
				...
				[(_workers select 0), "Upgrade finished", "Laboratory upgrade", mapGridPosition (getPos _labka)] remoteExec ["owr_fn_message", bis_curator_west];
